...
_logger = FakeLogger()
def Vim_window():
    vim.command('execute get(g:, "peekaboo_window", s:default_window)')
...
class VimOpen(Action):
    def __call__(self):
        ...
        # target = parse_path(self.target)
        target = self.target

        # TODO: make space handling more robust?
        # vim.command('e {}'.format(path.path.replace(' ', '\\ ')))
        # vim.command('execute get(g:, "peekaboo_window","vertical botright 30new")')
        mylist=[
        '# {}'.format(target),
        '## {}'.format(target),
        '### {}'.format(target),
